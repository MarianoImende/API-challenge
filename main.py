...
from documentacion import documentacion_bienvenida, documentacion_cuentas, documentacion_estado, documentacion_logout, documentacion_mov, documentacion_saldo, documentacion_sesion,documentacion_pago_qr
...
async def sesion(usuario: Usuario):
    ...
    if username == "challenge":
        json.update({"tarjetas": [{"descripcion": "NNOVA TRUST BANK", "numero": "825840853443", "estado":"Activa"}, {"descripcion": "TITANIUM FINANCE BANK", "numero": "423455721156", "estado":"pausada"}, {"descripcion": "ASTRA CAPITAL BANK", "numero": "595278769781", "estado":"Activa"}]})
    else:
...
async def saldo(cuenta: Cuenta , headers: Annotated[SesionHeaders, Header()], user: User = Depends(get_user_disable_current),token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
    ...
    if user.username == "challenge":
        if cuenta.numero_cuenta == "99083422":
            ...
            return {"saldo": Decimal('19209.10'),"moneda":"ARS"}
