...
if __name__ == "__main__":
    ...
    import uvicorn
    #ambiente de desa
    uvicorn.run(app="main:app",host="127.0.0.1", port=1976, log_level="debug",reload=True)
    
    #Ambiente de prod:
    uvicorn.run(app="main:app",host="127.0.0.1", port=1976, log_level="error",reload=False)
    #Para generar el EXE con Pyinstaller:
    #(venv) D:\MisProyectos\API-challenge>pyinstaller main.spec
    #se debe utilizar el archivo: D:\MisProyectos\API-challenge>main.spec (hay una copia:bkp-main.spec)
    # o bien:
""" a = Analysis(
    ['main.py'],
    pathex=['D:\\MisProyectos\\API-challenge','D:\\MisProyectos\\API-challenge\\venv\\Lib\\site-packages'],
    binaries=[],
    datas=[],
...
exe = EXE(
    ...
    upx_exclude=[],
    runtime_tmpdir=None,
    icon='D:\\MisProyectos\\API-challenge\\Api.ico',
    console=True,
...
coll = COLLECT(
 ...
 """
    ...
    #uvicorn.run(app="main:app",host="127.0.0.1", port=80, reload=True,log_level="info")  
    #pyinstaller --onefile --path="D:\MisProyectos\API-challenge\venv\Lib\site-packages" --hidden-import=passlib.handlers main.py
    
    #,{pyinstaller --onefile --path="D:\MisProyectos\API-challenge\venv\Lib\site-packages" --hidden-import=passlib.handlers.bcrypt main.py